...
mod integration_tests {
    ...
    mod data_acc_part {
        ...
        use sabi_redis::{sentinel::RedisDataConnAsync, RedisPubSubMsgDataConnAsync};
        ...
        #[overridable]
        pub(crate) trait RedisDataAccAsync: DataAcc {
            async fn send_greeting_message_async(&mut self, s: &str) -> errs::Result<()> {
                let data_conn = self
        ...
        #[overridable]
        pub(crate) trait RedisPubSubDataAccAsync: DataAcc {
            async fn receive_message_async(&mut self) -> errs::Result<String> {
                let data_conn = self
    ...
    mod controller_part {
        ...
        use redis::{sentinel::SentinelServerType, ControlFlow};
        use sabi::tokio::{logic, DataHub};
        use sabi_redis::{
            sentinel::{RedisDataSrcAsync, RedisPubSubSubscriberAsync},
