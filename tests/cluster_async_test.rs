...
mod integration_tests {
    ...
    use sabi_redis::RedisClusterDataSrcAsync;

    uses!(
        "redis",
        RedisClusterDataSrcAsync::new(&[
    ...
    mod data_acc_part {
        ...
        use sabi_redis::RedisClusterDataConnAsync;
        ...
        pub(crate) trait RedisSayingDataAccAsync: DataAcc {
            async fn say_greeting_async(&mut self, greeting: &str) -> errs::Result<()> {
                let data_conn = self
                    .get_data_conn_async::<RedisClusterDataConnAsync>("redis")
                    .await?;
