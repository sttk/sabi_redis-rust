...
impl DataSrc<RedisDataConnAsync> for RedisDataSrcAsync {
    async fn setup_async(&mut self, _ag: &mut AsyncGroup) -> errs::Result<()> {
        ...
        let pool_cfg = pool_opt.ok_or_else(|| errs::Err::new(RedisErrorAsync::AlreadySetup))?;
        match pool_cfg {
...
mod unit_tests_of_data_src {
    ...
    mod test_new {
        ...
        async fn addrs_are_urls_and_ok() {
            ...
            let mut ds = RedisDataSrcAsync::new(&[url0, url1, url2]);
            let mut ag = AsyncGroup::new();
            if let Err(err) = ds.setup_async(&mut ag).await {
                panic!("{err:?}");
            }
    ...
    mod test_with_pool_config {
        ...
        async fn addrs_are_urls_and_ok() {
            ...
            let mut ds = RedisDataSrcAsync::with_pool_config(&[url0, url1, url2], pool_cfg);
            let mut ag = AsyncGroup::new();
            if let Err(err) = ds.setup_async(&mut ag).await {
                panic!("{err:?}");
            }
