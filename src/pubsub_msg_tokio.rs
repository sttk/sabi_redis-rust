...
impl RedisPubSubMsgDataConnAsync {
...
impl DataConn for RedisPubSubMsgDataConnAsync {
...
impl DataSrc<RedisPubSubMsgDataConnAsync> for RedisPubSubMsgDataSrcAsync {
    ...
    async fn create_data_conn_async(&mut self) -> errs::Result<Box<RedisPubSubMsgDataConnAsync>> {
        let msg = Arc::clone(&self.msg);
        Ok(Box::new(RedisPubSubMsgDataConnAsync::new(msg)))
    }
}
...
mod unit_tests_of_data_src {
    ...
    mod test_new {
        ...
        async fn ok() {
            ...
            let mut ds = RedisPubSubMsgDataSrcAsync::new(msg);
