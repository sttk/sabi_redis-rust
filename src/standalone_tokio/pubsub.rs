    ...
    A: ToRedisArgs,
{
    ...
    {
        let addr = self.addr.take().ok_or_else(|| {
            errs::Err::new(RedisPubSubSubscriberErrorAsync::AddressAlreadyConsumed)
        })?;
        let client = match addr {
            RedisConfig::String(addr) => Client::open(addr.as_str()).map_err(|e| {
                errs::Err::with_source(
                    RedisPubSubSubscriberErrorAsync::FailToOpenClientOfAddr { addr },
                    e,
                )
            })?,
            RedisConfig::ConnAddr(conn_addr) => Client::open(conn_addr.clone()).map_err(|e| {
                errs::Err::with_source(
                    RedisPubSubSubscriberErrorAsync::FailToOpenClientOfConnAddr { conn_addr },
                    e,
                )
            })?,
            RedisConfig::ConnInfo(conn_info) => Client::open(conn_info.clone()).map_err(|e| {
                errs::Err::with_source(
                    RedisPubSubSubscriberErrorAsync::FailToOpenClientOfConnInfo { conn_info },
                    e,
                )
            })?,
        };
...
mod unit_tests {
    ...
    mod test_new {
        ...
        async fn addr_is_str_and_fail() {
            let mut subscriber = RedisPubSubSubscriberAsync::new("xxxx");
            ...
            let Ok(RedisPubSubSubscriberErrorAsync::FailToOpenClientOfAddr { addr }) =
        ...
        async fn addr_is_string_and_ok() {
            ...
            let mut subscriber = RedisPubSubSubscriberAsync::new("redis://127.0.0.1/0".to_string());
        ...
        async fn addr_is_string_and_fail() {
            let mut subscriber = RedisPubSubSubscriberAsync::new("xxxx".to_string());
        ...
        async fn addr_is_url_and_ok() {
            ...
            let mut subscriber = RedisPubSubSubscriberAsync::new(url);
    ...
    mod test_with_conn_addr {
        ...
        async fn ok() {
            ...
            let mut subscriber = RedisPubSubSubscriberAsync::with_conn_addr(conn_addr);
    ...
    mod test_with_conn_info {
        ...
        async fn ok() {
            ...
            let mut subscriber = RedisPubSubSubscriberAsync::with_conn_info(conn_info);
