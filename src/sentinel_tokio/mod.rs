        ...
        match pool {
            RedisPool::Config(config) => {
                ...
                pool.timeout_get(&timeouts).await.map_err(|e| {
                    errs::Err::with_source(RedisErrorAsync::FailToConnect { config: *config }, e)
                })?;
...
mod unit_tests_of_data_src {
    ...
    mod test_new {
        ...
        async fn addrs_are_urls_and_ok() {
            ...
            let mut ds =
                RedisDataSrcAsync::new(&[url0, url1, url2], "mymaster", SentinelServerType::Master);
            let mut ag = AsyncGroup::new();
            if let Err(err) = ds.setup_async(&mut ag).await {
                panic!("{err:?}");
            }
    ...
    mod test_with_config {
        ...
        async fn ok() {
            ...
            let mut ds = RedisDataSrcAsync::with_config(config);
            let mut ag = AsyncGroup::new();
            if let Err(err) = ds.setup_async(&mut ag).await {
                panic!("{err:?}");
            }
