    ...
    A: ToRedisArgs,
{
        ...
        F: FnMut(Msg) -> ControlFlow<U>,
    {
        let cfg = self.config.take().ok_or_else(|| {
            errs::Err::new(RedisPubSubSubscriberError::SentinelConfigAlreadyConsumed)
        })?;
        let mut client = match cfg {
            SentinelConfig::String(boxed_cfg) => {
                ...
                .map_err(|e| {
                    errs::Err::with_source(
                        RedisPubSubSubscriberError::FailToBuildSentinelClientOfAddrs {
                            addrs: cfg.addrs,
            ...
            SentinelConfig::ConnAddr(boxed_cfg) => {
                ...
                .map_err(|e| {
                    errs::Err::with_source(
                        RedisPubSubSubscriberError::FailToBuildSentinelClientOfConnAddrs {
            ...
            SentinelConfig::ConnInfo(boxed_cfg) => {
                ...
                .map_err(|e| {
                    errs::Err::with_source(
                        RedisPubSubSubscriberError::FailToBuildSentinelClientOfConnInfos {
            ...
            SentinelConfig::ClientBuilder(client_builder) => {
                client_builder.build().map_err(|e| {
                    errs::Err::with_source(
                        RedisPubSubSubscriberError::FailToBuildSentinelClientWithClientBuilder,
                        e,
                    )
                })?
            }
...
mod unit_tests {
    use super::*;
    use crate::sentinel::RedisDataSrc;
    ...
    mod test_new {
        ...
        fn addrs_are_strs_and_fail() {
            ...
            let Ok(RedisPubSubSubscriberError::FailToBuildSentinelClientOfAddrs {
                addrs,
                service_name,
                server_type,
    ...
    mod test_with_node_conn_info {
        ...
        fn addrs_are_strs_and_ok() {
            ...
            let mut subscriber = RedisPubSubSubscriber::with_node_conn_info(
                &[
                    "redis://127.0.0.1:26479",
                    "redis://127.0.0.1:26480",
                    "redis://127.0.0.1:26481",
    ...
    mod test_with_conn_addrs {
        ...
        fn ok() {
            ...
            let mut subscriber = RedisPubSubSubscriber::with_conn_addrs(
        ...
        fn fail() {
            ...
            let Ok(RedisPubSubSubscriberError::FailToBuildSentinelClientOfConnAddrs {
                conn_addrs,
                service_name,
                server_type,
            }) = err.reason::<RedisPubSubSubscriberError>()
    ...
    mod test_with_conn_addrs_and_node_conn_info {
        ...
        fn ok() {
            ...
            let node_conn_info = SentinelNodeConnectionInfo::default();
            let mut subscriber = RedisPubSubSubscriber::with_conn_addrs_and_node_conn_info(
                vec![conn_addr0, conn_addr1, conn_addr2],
                "mymaster",
                SentinelServerType::Master,
                node_conn_info,
    ...
    mod test_with_conn_infos {
        ...
        fn ok() {
            ...
            let mut subscriber = RedisPubSubSubscriber::with_conn_infos(
        ...
        fn fail() {
            ...
            let Ok(RedisPubSubSubscriberError::FailToBuildSentinelClientOfConnInfos {
                conn_infos,
                service_name,
                server_type,
            }) = err.reason::<RedisPubSubSubscriberError>()
    ...
    mod test_with_conn_infos_and_node_conn_info {
        ...
        fn ok() {
            ...
            let node_conn_info = SentinelNodeConnectionInfo::default();
            let mut subscriber = RedisPubSubSubscriber::with_conn_infos_and_node_conn_info(
                vec![conn_info0, conn_info1, conn_info2],
                "mymaster",
                SentinelServerType::Master,
                node_conn_info,
    ...
    mod test_with_client_builder {
        ...
        fn ok() {
            ...
            let mut subscriber = RedisPubSubSubscriber::with_client_builder(client_builder);
        ...
        fn fail() {
            ...
            let Ok(RedisPubSubSubscriberError::FailToBuildSentinelClientWithClientBuilder) =
                err.reason::<RedisPubSubSubscriberError>()
            else {
                panic!();
            };
